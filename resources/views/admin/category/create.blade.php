<x-app-layout>
    <section class="w-full m-auto bg-white shadow sm:rounded-lg p-4">
        <h1 class="text-xl font-bold">Thêm mới danh mục</h1>

        <form action="{{ route('admin.category.store') }}" method="post" class="mt-6 space-y-6">
            @csrf
            @method('post')

            <div>
                <x-input-label for="name" :value="__('Tiêu đề')" :isRequired="true"/>
                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full title-slug" required autofocus
                    autocomplete="name" />
            ...
            <div class="mt-4">
                <div class="flex items-center mb-4">
                    <input id="default-checkbox" type="checkbox" name="is_active" :value="ACTIVE"
                        {{-- @if ($cateo->is_active == ACTIVE) checked @endif --}}
